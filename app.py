...
def render_booking(id_teacher, week_day, time):
    ...
    edited_schedule = Schedule.query.filter(
        db.and_(Schedule.id_teacher == id_teacher, Schedule.weekday == day_reverse(week_day),
...
def render_request_done():
    ...
    goal_request = GoalsRequest(goal=form.goal.data, time=int(form.spend_time.data), name=form.name.data,
                                phone=form.phone.data)
    db.session.add(goal_request)
    db.session.commit()
    data['goal'] = Goal.query.get(form.goal.data)
    data['spend_time'] = form.spend_time.data + 'часов'
